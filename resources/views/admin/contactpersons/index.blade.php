...
@can('contactperson_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.contactpersons.create') }}">
                        ...
                        <tr data-entry-id="{{ $contactperson->id }}">
                            ...
                            <td>
                                @can('contactperson_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.contactpersons.show', $contactperson->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('contactperson_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.contactpersons.edit', $contactperson->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('contactperson_delete')
                                    <form action="{{ route('admin.contactpersons.destroy', $contactperson->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('contactperson_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.contactpersons.massDestroy') }}",
