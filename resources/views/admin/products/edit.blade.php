...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.products.update", [$product->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="bahan">{{ trans('cruds.product.fields.bahan') }}</label>
                <input class="form-control {{ $errors->has('bahan') ? 'is-invalid' : '' }}" type="text" name="bahan" id="bahan" value="{{ old('bahan', $product->bahan) }}">
                @if($errors->has('bahan'))
                    <div class="invalid-feedback">
                        {{ $errors->first('bahan') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.product.fields.bahan_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="fungsi">{{ trans('cruds.product.fields.fungsi') }}</label>
                <input class="form-control {{ $errors->has('fungsi') ? 'is-invalid' : '' }}" type="text" name="fungsi" id="fungsi" value="{{ old('fungsi', $product->fungsi) }}">
                @if($errors->has('fungsi'))
                    <div class="invalid-feedback">
                        {{ $errors->first('fungsi') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.product.fields.fungsi_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="customer">{{ trans('cruds.product.fields.customer') }}</label>
                <input class="form-control {{ $errors->has('customer') ? 'is-invalid' : '' }}" type="text" name="customer" id="customer" value="{{ old('customer', $product->customer) }}">
