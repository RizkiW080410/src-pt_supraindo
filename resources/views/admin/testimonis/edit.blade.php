    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.testimonis.update", [$testimoni->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="name">{{ trans('cruds.testimoni.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $testimoni->name) }}">
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.testimoni.fields.name_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    ...
    url: '{{ route('admin.testimonis.storeMedia') }}',
    maxFilesize: 2, // MB
...
@if(isset($testimoni) && $testimoni->image)
      var file = {!! json_encode($testimoni->image) !!}
